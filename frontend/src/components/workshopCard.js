...
const WorkshopCard = ({
  ...
  useEffect(() => {
    const tokenAmount = Number(price)
    const maxDiscountTokens = Math.min(balance, 200)
    const discountAmount = Math.min(maxDiscountTokens / 100, tokenAmount - 1)
    const discountedPrice = tokenAmount - discountAmount
  ...
  const handleBuyWorkshop = async () => {
    ...
    try {
      if (!walletAddress) {
        toast.warning('Please sign in to purchase this workshop')
        return
      }

      await fetchBalance()
      await toast.promise(
        purchaseWorkshop(soulToken, pyusdToken, workshop?._id),
        {
        ...
        <div className='mt-auto'>
          ...
          {!isCreator && !purchased ? (
            <button
              ...
              <FaShoppingCart className='mr-2' /> {walletAddress ? 'Buy Workshop' : 'Sign in to Purchase'}
            </button>
          ) : purchased ? (
